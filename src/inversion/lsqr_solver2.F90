...
subroutine lsqr_solve_sensit(nlines, ncolumns, niter, rmin, gamma, target_misfit, &
  ...
  real(kind=CUSTOM_REAL) :: Bk2, Dk2, cond
  ...
  do while (iter <= niter .and. r > rmin)
    ...
    Bk2 = Bk2 + alpha**2 + beta**2
    Dk2 = Dk2 + sum(w**2) * rho_inv**2
  ...
  call MPI_Allreduce(MPI_IN_PLACE, Dk2, 1, CUSTOM_MPI_TYPE, MPI_SUM, MPI_COMM_WORLD, ierr)
