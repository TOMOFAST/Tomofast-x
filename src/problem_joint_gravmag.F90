...
subroutine solve_problem_joint_gravmag(gpar, mpar, ipar, myrank, nbproc)
  ...
  if (SOLVE_PROBLEM(1)) call model(1)%grid_full%allocate(ipar%nx, ipar%ny, ipar%nz, gpar%z_axis_dir, myrank)
  ...
  if (SOLVE_PROBLEM(1)) call data(1)%initialize(gpar%ndata, gpar%ndata_components, gpar%data_units_mult, gpar%z_axis_dir, myrank)
  if (SOLVE_PROBLEM(2)) call data(2)%initialize(mpar%ndata, mpar%ndata_components, mpar%data_units_mult, mpar%z_axis_dir, myrank)
  ...
  if (SOLVE_PROBLEM(1)) &
    call model(1)%initialize(ipar%nelements, ipar%nmodel_components, allocate_full_model_on_all_cpus(1), &
                             gpar%model_units_mult, gpar%vtk_model_label, myrank, nbproc)
  if (SOLVE_PROBLEM(2)) &
...
! Calculate data residuals.
!==================================================================================================
pure subroutine calculate_residuals(ndata, ncomponents, data_obs, data_calc, data_weight, residuals)
...
!  enddo
  ...
  ! Switched the data order.

  p = 1
  do d = 1, ncomponents
    do i = 1, ndata
      residuals(p) = (data_obs(d, i) - data_calc(d, i)) * data_weight(i)
      p = p + 1
    enddo
  enddo
