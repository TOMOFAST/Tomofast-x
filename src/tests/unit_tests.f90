...
subroutine test_all(myrank, nbproc)
  ...
  ! Parallel tests:
  call test(test_get_number_elements_on_other_cpus, "Test of getting number of elements on other cpus.", myrank, nbproc)
  call test(test_get_total_number_elements, "Test of getting total number of elements.", myrank, nbproc)
  call test(test_get_full_array, "Test of getting the full array.", myrank, nbproc)
  call test(test_get_full_array_in_place, "Test of getting the full array in place.", myrank, nbproc)

  call test(test_add_damping_identity_matrix, "Test of adding damping identity matrix.", myrank, nbproc)
  call test(test_cross_gradient_calculate_all, "Tests of calculating cross gradient matrix.", myrank, nbproc)
