...
subroutine test_add_damping_identity_matrix(myrank, nbproc)
  ...
  call model%initialize(par%nelements, par%nmodel_components, .true., 1.d0, "F", myrank, nbproc)
  ...
  call damping%add(isensit, isensit%get_total_row_number(), b_RHS, arr%column_weight, &
                   model%val(:, 1), model%val_prior(:, 1), 0, .true., myrank, nbproc)
...
subroutine test_cross_gradient_calculate(myrank, nbproc, derivative_type)
  ...
  call model1%initialize(nelements_total, ncomponents, .true., 1.d0, "F", myrank, nbproc)
  call model1%grid_full%allocate(nx, ny, nz, 1, myrank)

  call model2%initialize(nelements_total, ncomponents, .true., 1.d0, "F", myrank, nbproc)
  call model2%grid_full%allocate(nx, ny, nz, 1, myrank)
  ...
  call cross_grad%initialize(nx, ny, nz, nelements, keep_model_constant, vec_field_type, vec_field_file, myrank)
