...
module parameters_gravmag
  ...
  type, public :: t_parameters_base
    ...
    ! (1): Synthetic model (for forward calc).
    ! (2): Prior model.
    ! (3): Starting model.
    character(len=256) :: model_files(3)
    character(len=256) :: model_grid_file

    ! Calculate forward data from synthetic model.
    integer :: calc_forward
    ...
    !------ Error ----------------------------------------------------------
    integer :: use_data_error
    character(len=256) :: data_error_file
...
subroutine parameters_base_broadcast(this, myrank)
  ...
  call MPI_Bcast(this%calc_forward, 1, MPI_INTEGER, 0, MPI_COMM_WORLD, ierr)
  ...
  call MPI_Bcast(this%use_data_error, 1, MPI_INTEGER, 0, MPI_COMM_WORLD, ierr)
  call MPI_Bcast(this%data_error_file, 256, MPI_CHARACTER, 0, MPI_COMM_WORLD, ierr)
