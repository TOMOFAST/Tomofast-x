...
!
!     Vitaliy Ogarko: Adjusted to support parallel tests (MPI).
!
module ftnunit
    use global_typedefs
...
subroutine test( proc, text, myrank, nbproc )
    ...
    if ( testno <= last_test ) then
        print *, 'Skipping test number =', testno, ', last test =', last_test, ', myrank =', myrank
        return
    endif
...
!     do the actual tests.
!
subroutine runtests_final(myrank)
  integer, intent(in) :: myrank
  integer :: exist_loc, exist_glob, ierr
  integer :: nofails_glob
  ...
  ! Gather the total number of fails from all CPUs.
  call mpi_allreduce(nofails, nofails_glob, 1, MPI_INTEGER, MPI_SUM, MPI_COMM_WORLD, ierr)
  nofails = nofails_glob

  if (exist_glob > 0) then
  ! At least one CPU found the "ftnunit.run" file (before it gets removed).

    if (myrank == 0) then
      print *, '******************************************************************'
      write(*,'(a,i5)') 'Number of failed assertions:                ', nofails
      write(*,'(a,i5)') 'Number of runs needed to complete the tests:', noruns
      write(*,'(a,i5)') 'Total number of tests were run:             ', testno
      print *, '******************************************************************'

      call ftnunit_remove_file( "ftnunit.lst" )
      call ftnunit_remove_file( "ftnunit.run" )
    endif
...
!
subroutine runtests( testproc, myrank, nbproc )
  interface
    subroutine testproc(myrank, nbproc)
      integer, intent(in) :: myrank, nbproc
  ...
  if ( ftnunit_file_exists("ftnunit.run") ) then
    ...
    call MPI_Bcast(last_test, 1, MPI_INTEGER, 0, MPI_COMM_WORLD, ierr)
    call MPI_Bcast(nofails, 1, MPI_INTEGER, 0, MPI_COMM_WORLD, ierr)
    call MPI_Bcast(noruns, 1, MPI_INTEGER, 0, MPI_COMM_WORLD, ierr)

    noruns = noruns + 1

    call testproc(myrank, nbproc)
...
subroutine assert_comparable_real( value1, value2, margin, text )
    real(kind=CUSTOM_REAL), intent(in)             :: value1
    real(kind=CUSTOM_REAL), intent(in)             :: value2
    real(kind=CUSTOM_REAL), intent(in)             :: margin
    character(len=*), intent(in) :: text
...
subroutine assert_comparable_real1d( array1, array2, margin, text )
    real(kind=CUSTOM_REAL), dimension(:), intent(in)    :: array1
    real(kind=CUSTOM_REAL), dimension(:), intent(in)    :: array2
    real(kind=CUSTOM_REAL), intent(in)                  :: margin
    character(len=*), intent(in)      :: text
